...
def handle_value_error_for_single_csv_import(config_tuple: tuple, list_of_headers_from_configfile: object) -> object:
...
def import_single_file_as_UTF(config_file_path: str, file_identifier: str):
    ...
    return handle_value_error_for_single_csv_import(config_tuple, list_of_headers_from_configfile)


def import_all_config_specified_csv(file_list: str, config_file_path: str) -> dict:
    ...
    try:
        for x in file_list:
            imported_csv = import_single_file_as_UTF(config_file_path, x)
